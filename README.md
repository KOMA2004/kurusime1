...
自身の楽曲をダウンロードするにはYoutubeなどの概要欄から外部サイトへ遷移する必要がありました。

そこで「再生からダウンロードまでを一つのページで完結できる状態」
を目指し、本アプリを制作しました。
...
シークバーや音量フェーダーなどで使いまわせるようにしました。

また、App.tsxが長くならないよう、UIコンポーネントに表示とロジックを担当してもらう構造にしました。
...
これから楽曲数を増やすことを考え、楽曲を外部のデータベースで管理することにしました。

その際、サーバーダウンなどの予期せぬエラーに対応できるよう、エラーも検出できるようにしました。
...
* 独自ドメインでの公開
* SNS導線の追加
* jestとの互換性が厳しいChakraUIv3からサポート豊富なShadcnに移行する
